...
To prevent accidental exposure of sensitive data (passwords, API keys, session tokens) in the dashboard or JSON reports, the system employs a recursive scrubbing strategy.
...
- **State Backup**: Before local state removal, `terraform.tfstate` is backed up as `terraform.tfstate.backup.<timestamp>` when the file exists.
...
- **State Boundary**: `terraform state rm` removes Terraform tracking only. It does not destroy, stop, or deallocate a cloud resource.

## 5. Log Security

Operational logs (`audit.log`) are stored in a designated `logs/` directory, which is excluded from public web hosting paths. Access to these logs is strictly mediated via a secure local API (`dashboard_server.py`), preventing unauthorized public access via GitHub Pages or simple directory traversal.
